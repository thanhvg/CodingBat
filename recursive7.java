...
// somewhere a value followed in the array by that value times
...
public boolean array220(int[] nums, int index) {
  ...
  boolean aux = (nums[index]*10 == nums[index+1]);
  if (index == nums.length - 2) return aux;
  return aux || array220(nums, index+1);
}

// note that nums[index] == nums[index+1]/10 is not ok
